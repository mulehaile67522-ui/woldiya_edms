...
Back up the SQLite database and uploaded documents before maintenance or moving the project:

```bash
python manage.py backup_data
```

Backups are written to `backups/`. You can also double-click `BACKUP_EDMS.bat` on Windows. Keep a copy of each backup outside the project folder.

## 🚂 Deploy to Railway

1. Create a Railway project and connect this repository.
2. Add a PostgreSQL database and connect it to the web service so Railway provides `DATABASE_URL`.
3. Add these service variables:
   - `SECRET_KEY`: a long random value
   - `DEBUG`: `False`
   - `ALLOWED_HOSTS`: your Railway hostname, such as `woldiya-edms.up.railway.app`
   - `CSRF_TRUSTED_ORIGINS`: `https://` followed by the same hostname
   - `DJANGO_SUPERUSER_USERNAME`, `DJANGO_SUPERUSER_PASSWORD`, and `DJANGO_SUPERUSER_EMAIL`
4. Deploy. Railway runs migrations, collects static files, creates the initial admin, and starts Gunicorn automatically.

Uploaded documents are stored on the application filesystem. Railway volumes or an object-storage backend are required if uploaded files must survive redeploys or multiple instances.
